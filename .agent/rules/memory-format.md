...
- cn() utility: always use lib/utils/cn.ts for merging Tailwind classes in components that accept className props. Never concatenate class strings manually.
...
- NEVER install Shadcn, Radix, Headless UI, MUI, Chakra, or Flowbite.
  Exception: lucide-react is approved for icons only. Do not use any other icon library.
